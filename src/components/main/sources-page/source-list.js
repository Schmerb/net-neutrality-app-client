...
export default class SourcesList extends Component {
  ...
  render() {
    const candidates = this.props.candidates
      .filter(
        candidate =>
          !(candidate.source === '' || candidate.source.trim() === 'sent')
      )
      .map((candidate, key) => {
        return (
    ...
    return (
      <section
        className={`${this.props.group}-sources sources`}
        id={`${this.props.group}-sources`}
      >
        <h4 className={this.state.classes}>
          {this.props.group.charAt(0).toUpperCase() + this.props.group.slice(1)}
        </h4>
