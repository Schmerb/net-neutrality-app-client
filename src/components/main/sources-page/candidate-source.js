...
export default class CandidateSource extends Component {
    ...
    if (src.includes('twitter.com')) {
      src = (
        <Link to={src} target="_blank">
          <span className="aria-hidden" aria-hidden="false">
            Navigate to {this.props.candidate.firstName}{' '}
            {this.props.candidate.lastName}'s twitter source
          </span>
      ...
      src = (
        <Link to={src} target="_blank">
          <span className="aria-hidden" aria-hidden="false">
            Navigate to {this.props.candidate.firstName}{' '}
            {this.props.candidate.lastName}'s facebook source
          </span>
      ...
      src = (
        <Link to={src} target="_blank">
          <span className="aria-hidden" aria-hidden="false">
            Navigate to {this.props.candidate.firstName}{' '}
            {this.props.candidate.lastName}'s imgur source
          </span>
      ...
      src = (
        <Link to={src} target="_blank">
          <span className="aria-hidden" aria-hidden="false">
            Navigate to {this.props.candidate.firstName}{' '}
            {this.props.candidate.lastName}'s external source
          </span>
      ...
      src.includes('Facebook post') ||
      src.includes('Facebook message') ||
      src.includes('Facebook response')
    ) {
      src = this.formatFbSrc(src);
    } else if (
      src.includes('Email response:') ||
      src.includes('Email response')
    ) {
  ...
  render() {
    ...
    return (
      <div
        className={`candidate-source ${this.state.classes} ${
          hasIcon ? 'hasIcon' : ''
        } ${hasQuote ? 'hasQuote' : ''}`}
      >
        <h3>
