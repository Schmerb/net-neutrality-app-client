...
export class CandidatesContainer extends Component {
  ...
  getAllStates = currentState => {
    return entries(states).map((state, key) => (
      <option key={key} value={state[1]}>
        {state[0]}
      </option>
    ));
  };
        ...
        <div className="candidates-inner-wrap">
          ...
          {!this.props.display ? (
            <p className="directions">
              Select your state to see which candidates support net neutrality
            </p>
          ) : null}
          ...
          <ListsContainer
            senate={senate}
            house={house}
            className={!this.props.display ? 'hide' : ''}
          />
        </div>
      </div>
    );
